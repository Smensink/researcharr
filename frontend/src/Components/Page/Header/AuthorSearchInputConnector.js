...
function createMapDispatchToProps(dispatch, props) {
  return {
    onGoToAuthor(titleSlug) {
      dispatch(push(`${window.Researcharr.urlBase}/author/${titleSlug}`));
    },

    onGoToBook(titleSlug) {
      dispatch(push(`${window.Researcharr.urlBase}/book/${titleSlug}`));
    },

    onGoToAddNewAuthor(query) {
      dispatch(push(`${window.Researcharr.urlBase}/add/search?term=${encodeURIComponent(query)}`));
    }
  };
}
