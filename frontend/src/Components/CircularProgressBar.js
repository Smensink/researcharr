...
const CircularProgressBar = ({
  ...
  // These values only change when props change, so we can avoid re-calculating them on every single animation frame.
  ...
  useEffect(() => {
    setProgress(0); // Reset animation when target progress changes

    const progressStep = () => {
      setProgress((currentProgress) => {
        if (currentProgress < targetProgress) {
          const nextProgress = currentProgress + 1;
          requestAnimationFrameId.current = window.requestAnimationFrame(progressStep);
          return nextProgress;
        }
