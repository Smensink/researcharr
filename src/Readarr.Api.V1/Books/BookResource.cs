    ...
    public static class BookResourceMapper
    {
        public static BookResource ToResource(this Book model)
        {
            ...
            return new BookResource
            {
                ...
                Journal = selectedEdition?.Disambiguation,
                Doi = model.Links.Concat(selectedEdition?.Links ?? Enumerable.Empty<Links>()).FirstOrDefault(x => x.Name.Equals("DOI", StringComparison.OrdinalIgnoreCase))?.Url,
                Citations = model.Ratings?.Votes ?? selectedEdition?.Ratings?.Votes ?? 0,
                Added = model.Added,
