...
namespace NzbDrone.Core.Books
{
        ...
        IExecute<BulkRefreshAuthorCommand>
    {
        ...
        {
            var works = remote?.Books?.Value ?? new List<Book>();

            // Allow all works from OpenAlex for now; metadata filtering can be reintroduced with DOI-aware rules if needed.
            var distinct = works.DistinctBy(m => m.ForeignBookId).ToList();
            var ids = distinct.Select(x => x.ForeignBookId).ToList();
            var excluded = _importListExclusionService.FindByForeignId(ids).Select(x => x.ForeignId).ToList();
            return distinct.Where(x => !excluded.Contains(x.ForeignBookId)).ToList();
        }
