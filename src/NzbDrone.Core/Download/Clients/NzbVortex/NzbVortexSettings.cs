...
namespace NzbDrone.Core.Download.Clients.NzbVortex
{
    ...
    public class NzbVortexSettings : IProviderConfig
    {
        ...
        [FieldDefinition(4, Label = "Group", Type = FieldType.Textbox, HelpText = "Adding a category specific to Researcharr avoids conflicts with unrelated non-Researcharr downloads. Using a category is optional, but strongly recommended.")]
        public string MusicCategory { get; set; }

        [FieldDefinition(5, Label = "Recent Priority", Type = FieldType.Select, SelectOptions = typeof(NzbVortexPriority), HelpText = "Priority to use when grabbing papers published within the last 14 days")]
        public int RecentTvPriority { get; set; }

        [FieldDefinition(6, Label = "Older Priority", Type = FieldType.Select, SelectOptions = typeof(NzbVortexPriority), HelpText = "Priority to use when grabbing papers published over 14 days ago")]
