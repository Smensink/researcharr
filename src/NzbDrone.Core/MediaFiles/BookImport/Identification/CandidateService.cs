...
namespace NzbDrone.Core.MediaFiles.BookImport.Identification
{
    ...
    public class CandidateService : ICandidateService
    {
        ...
        public IEnumerable<CandidateEdition> GetRemoteCandidates(LocalEdition localEdition, IdentificationOverrides idOverrides)
        {
            ...
            // DOI is the highest priority identifier for academic papers
            var dois = localEdition.LocalBooks
                .Select(x => DoiUtility.Normalize(x.FileTrackInfo.Doi))
                .Where(x => x.IsNotNullOrWhiteSpace())
                .Distinct()
                .ToList();

            if (dois.Count == 1 && dois[0].IsNotNullOrWhiteSpace())
            {
                _logger.Trace($"Searching by DOI {dois[0]}");

                try
                {
                    remoteBooks = _bookSearchService.SearchByDoi(dois[0]);
                }
                catch (Exception e)
                {
                    _logger.Info(e, "Skipping DOI search due to Error");
                ...
                // If DOI search found results, we're done - DOI is a unique identifier
                if (seenCandidates.Any())
                {
                    _logger.Debug("DOI search found candidates, skipping other searches");
                    yield break;
                }
            }
            ...
            if (isbns.Count == 1 && isbns[0].IsNotNullOrWhiteSpace())
            {
                ...
                {
                    _logger.Info(e, "Skipping ISBN search due to Error");
                    remoteBooks = new List<Book>();
                }
                ...
                asins[0].Length == 10)
            {
                ...
                {
                    _logger.Info(e, "Skipping ASIN search due to Error");
                    remoteBooks = new List<Book>();
                }
            ...
            {
                _logger.Info(e, "Skipping book title search due to Error");
                remoteBooks = new List<Book>();
            }
