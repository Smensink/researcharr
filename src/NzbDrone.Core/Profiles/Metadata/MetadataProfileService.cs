...
namespace NzbDrone.Core.Profiles.Metadata
{
    ...
    public class MetadataProfileService : IMetadataProfileService, IHandle<ApplicationStartedEvent>
    {
        ...
        private List<Edition> FilterEditions(IEnumerable<Edition> editions, List<Edition> localEditions, List<BookFile> localFiles, MetadataProfile profile)
        {
            ...
            FilterByPredicate(hash, x => x.ForeignEditionId, localHash, profile, (x, p) => !p.SkipMissingIsbn || x.Isbn13.IsNotNullOrWhiteSpace() || x.Asin.IsNotNullOrWhiteSpace() || x.Links.Any(l => l.Name.Equals("DOI", StringComparison.OrdinalIgnoreCase)), "isbn/asin/doi is missing");
        ...
        private bool BookAllowedByRating(Book b, MetadataProfile p)
        {
            ...
            var hasDoi = b.Links.Any(l => l.Name.Equals("DOI", StringComparison.OrdinalIgnoreCase));

            // Papers often lack traditional ratings; allow them through when they have DOI metadata
            if (hasDoi)
            {
                return true;
            }
