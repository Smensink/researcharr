...
SyncFrontendOutput()
{
    local uiSource="$outputFolder/UI"
    local targetBase="$outputFolder/net6.0"
    ...
    for rid in osx-arm64 linux-x64 win-x64; do
        if [ -d "$targetBase/$rid" ]; then
            rm -rf "$targetBase/$rid/UI"
            cp -R "$uiSource" "$targetBase/$rid/"
        fi

        if [ -d "$targetBase/$rid/publish" ]; then
            rm -rf "$targetBase/$rid/publish/UI"
            cp -R "$uiSource" "$targetBase/$rid/publish/"
        fi
    done
}
