...
- The LibGen search plugin reads mirror endpoints from the `libgen_mirrors` array in `config.json`. You can supply bare hosts (both `/index.php` and `/search.php` will be tested) or explicit entries with `url`/`params_type`. Use `python scripts/test_libgen_mirrors.py` to verify reachability.
- Stay tuned for upcoming features, including more plugin support and enhancements!
- **Testing Selenium plugins**: run the parsing-only suite with `pytest tests/test_selenium_plugins.py`. To exercise the real Selenium scrapers on your own machine (requires Chrome/Chromedriver and outbound network), export `RUN_SELENIUM_LIVE=1` and run `pytest tests/test_selenium_plugins_live.py`. Use `-k <plugin>` to focus on a single source when debugging.
