...
# Install gosu and chromium for selenium
RUN apt-get update && \
...
RUN mkdir -p /data/downloads/downloadarr

# Install dependencies from requirements.txt
RUN pip install --no-cache-dir -r requirements.txt

# Persist config and download directories as volumes
VOLUME ["/config", "/data/downloads/downloadarr"]
