...
const removeClassnamesImport = {
  ImportDeclaration(path) {
    ...
    if (type !== 'StringLiteral' || value !== CLASSNAMES_IMPORT && value !== CLASSNAMES_BIND_IMPORT) {
      return;
    }
...
const isIdentifierDefinedAs = (name, importName, scope) => {
  ...
  if (path.parent.type !== 'ImportDeclaration' || path.parent.source.value !== importName) {
    return false;
  }
...
const BabelPluginInlineClassnames = ({
  types: t
}) => {
  let boundName;
  let boundSource;
  ...
  function isValidNode(node) {
    if (t.isNullLiteral(node) || t.isIdentifier(node, {
      name: 'undefined'
    })) {
      return false;
    }
  ...
  function wrapIdentifier(node) {
    if (t.isIdentifier(node) || t.isMemberExpression(node) || t.isLogicalExpression(node) || t.isConditionalExpression(node)) {
  ...
  const wrapBound = (value, bound) => t.memberExpression(t.identifier(bound), t.identifier(value));
  ...
  function joinNodes(nodes, index, transformer, joiner, bound = null) {
    ...
    return createPlus(createPlus(wrapIdentifier(left), t.stringLiteral(' ')), wrapIdentifier(right));
  }
  ...
  function transformProperty(property, bound = null) {
    ...
    if (t.isNullLiteral(property.value) || t.isIdentifier(property.value, {
      name: 'undefined'
    })) {
      return t.stringLiteral('');
    }
  ...
  function joinObjectProperties(properties, bound = null) {
    return joinNodes(properties, 0, transformProperty, joinObjectProperties, bound);
  }
  ...
  const visitor = {
    ...
    VariableDeclaration(path) {
      ...
      const found = node.declarations.filter(declaration => declaration.type === 'VariableDeclarator' && declaration.init && declaration.init.type === 'CallExpression').some(declaration => {
